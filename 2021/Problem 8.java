...
// Never do this in Java. You think it'll be cool to do a bunch of different languages, but it's just painful.
// Just look at the first class I had to declare.
class TwoWayHashMap
{
    HashMap<Set<Character>,Integer> solution = new HashMap();
    HashMap<Integer,Set<Character>> reverseSolution = new HashMap();

    public TwoWayHashMap() {

    }
    public static String view(Set<Character> set) {
        Character[] str = set.toArray(new Character[0]);
        char[] st = new char[str.length];
        for(int i = 0; i < str.length; i++) st[i] = str[i];  
        return new String (st);
    }

    public void add(Set<Character> key, int value) {
        System.out.println(String.format("Saving %s %x", view(key), value));
        solution.put(key, value);
        reverseSolution.put(value, key);
    }
    public int get(Set<Character> key) {
        System.out.println(String.format("Getting %s", view(key)));
        return solution.get(key);
    }
    public Set<Character> get(int value) {
        return reverseSolution.get(value);
    }
}

class PuzzleEntry
{
    static List<Integer> DIGITS_WITH_UNIQUE_SEGMENTS = Arrays.stream(new int[] { 2, 3, 4, 7 }).boxed().collect(Collectors.toList());
    public int numberOfEasyDigits = 0;
    private String[] signalPatterns;
    private String[] outputValues;

    // entryText: be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
    public PuzzleEntry(String entryText)
    {
        String[] splitInput = entryText.split(" \\| ");
        //System.out.print("splitInput[1]:" + splitInput[1]);

        signalPatterns = splitInput[0].split(" ");

        outputValues = splitInput[1].split(" ");
        numberOfEasyDigits = (int)Arrays.stream(outputValues).filter(outputValue -> DIGITS_WITH_UNIQUE_SEGMENTS.contains(outputValue.length())).count();
        
        //System.out.print("outputValues[1]:" + outputValues[1]);
        //System.out.print("numberOfEasyDigits:" + numberOfEasyDigits);
    }

    public int getOutput() {
        TwoWayHashMap solution = new TwoWayHashMap();
        Set<Set<Character>> fiveSegmentSignals = new HashSet<Set<Character>>();
        Set<Set<Character>> sixSegmentSignals = new HashSet<Set<Character>>();

        for(String signalPattern : signalPatterns) {
            if (signalPattern.length() == 2) {
                solution.add(toSet(signalPattern), 1);
            }
            else if (signalPattern.length() == 4) {
                solution.add(toSet(signalPattern), 4);
            }
            else if (signalPattern.length() == 3) {
                solution.add(toSet(signalPattern), 7);
            }
            else if (signalPattern.length() == 7) {
                solution.add(toSet(signalPattern), 8);
            }
            else if (signalPattern.length() == 6) {
                sixSegmentSignals.add(toSet(signalPattern));
            }
            else if (signalPattern.length() == 5) {
                fiveSegmentSignals.add(toSet(signalPattern));
            }
        }

        Set<Character> b_and_d = new HashSet<>(solution.get(4));
        b_and_d.removeAll(solution.get(1));
        System.out.println(String.format("b and d segments: %s", TwoWayHashMap.view(b_and_d)));

        // Solve for the 6 segment ones (0, 6, 9)

        // 0 is the only 6-segment digit that does not contain both segments b and d (4-1).
        solution.add(sixSegmentSignals
            .stream()
            .filter(signal -> !signal.containsAll(b_and_d))
            .findFirst()
            .get(),
            0);

        // 6 is the only 6-segment digit that does not contain both segments c and f (1).
        solution.add(sixSegmentSignals
            .stream()
            .filter(signal -> !signal.containsAll(solution.get(1)))
            .findFirst()
            .get(),
            6);

        // 9 is the other 6-segment digit.
        solution.add(sixSegmentSignals
            .stream()
            .filter(signal -> !signal.equals(solution.get(0)) && !signal.equals(solution.get(6)))
            .findFirst()
            .get(),
            9);

        // Solve for the 5 segment ones (5, 3, 2)

        // 5 is the only 5-segment digit that does contains both segments b and d (4-1).
        solution.add(fiveSegmentSignals
            .stream()
            .filter(signal -> signal.containsAll(b_and_d))
            .findFirst()
            .get(),
        ...
        // 3 is the only 5-segment digit that contains both segments c and f (1).
        solution.add(fiveSegmentSignals
            .stream()
            .filter(signal -> signal.containsAll(solution.get(1)))
            .findFirst()
        ...
        // 2 is the other 5-segment digit.
        solution.add(fiveSegmentSignals
            .stream()
            .filter(signal -> !signal.equals(solution.get(5)) && !signal.equals(solution.get(3)))
            .findFirst()
            .get(),
            2);
        ...
        for(String digit : outputValues) {
            output += solution.get(toSet(digit));
            output *= 10;
        }
...
class PuzzleInput
{
    private List<PuzzleEntry> entries;
    public PuzzleInput(String rawInputText)
    {
        entries = Arrays.stream(rawInputText.split(System.getProperty("line.separator")))
            .map(entryLine -> new PuzzleEntry(entryLine))
            .collect(Collectors.toList());
    }
    public int getNumberOfEasyDigits() {
        return entries.stream().map(entry -> entry.numberOfEasyDigits).mapToInt(Integer::intValue).sum();
    }
    public int getSumOfOutputs() {
        return entries.stream().map(entry -> entry.getOutput()).mapToInt(Integer::intValue).sum();
    }
}
...
public class Main
{
    public static void main(String[] args)
    {
        ...
        System.out.println("getNumberOfEasyDigits: " + new PuzzleInput(x).getNumberOfEasyDigits());
        System.out.println("getSumOfOutputs: " + new PuzzleInput(x).getSumOfOutputs());

        String realInput = String.join(
            System.getProperty("line.separator"),
        ...
        System.out.println("getNumberOfEasyDigits: " + new PuzzleInput(realInput).getNumberOfEasyDigits());
        System.out.println("getSumOfOutputs: " + new PuzzleInput(realInput).getSumOfOutputs());
    }
}