...
def apply_new_region(existing_regions, region_to_add):
    ...
    if debug: print("*Starting existing regions:\n%s" % RegionV2.list_str(existing_regions))
    for existing_region in existing_regions:
        ...
        if debug: print("*Ending new regions:\n%s" % RegionV2.list_str(new_regions))
    
    rechopped_existing_regions = list(set(rechopped_existing_regions))
    if debug: print("*Ending rechopped regions:\n%s" % RegionV2.list_str(rechopped_existing_regions))
    return ([nr for nr in new_regions if nr.is_on], [r for r in rechopped_existing_regions if r.is_on])
...
def find_cube_on_count(input_regions):
    ...
    for region_to_merge in input_regions[1:]:
        ...
        for region_to_recalculate in regions_to_recalculate:
            new_regions_updated = []
            for new_region in new_regions:
                (nr, ror) = apply_new_region([region_to_recalculate], new_region)
                new_regions_updated.extend(nr)
                rechopped_older_regions.extend(ror)
        ...
        nonoverlapping_regions = remove_nested_regions(set(nonoverlapping_regions))
